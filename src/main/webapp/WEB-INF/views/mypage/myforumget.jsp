		...
		replyService.getList({fno : fnoValue,page : page || 1},
							...
							if(page==-1){//마지막페이지 찾아서 호출
								...
								for (var i = 0, len = list.length || 0; i < len; i++) {
									...
									str += "<div><div class='header'style='height:50px'><strong class='primary-font'>["
											+ list[i].rno+"] "+list[i].replyer+"<img style='width:20px;' src='/resources/img/writer.png'>"+ "</strong>";
									}else{
										str += "<div><div class='header'style='height:50px'><strong class='primary-font'>["
		...
		<div class="panel panel-default">
			...
			<table>
				<tr>
					...
					<td><input class="form-control" name="bno" style='width:100px;margin-right:10px;margin-left:10px;'
						value='<c:out value="${forum.bno}"/>' readonly="readonly"></td>
					<td><label>등록일</label></td>
					<td><input class="form-control" name="regDate" style='width:150px;margin-right:10px;margin-left:10px;'
						value='<fmt:formatDate pattern="yyyy-MM-dd hh:mm"
									value="${forum.regdate }" />' readonly="readonly"></td>
					<td><label>수정일</label></td>
					<td><input class="form-control" name="updateDate" style='width:150px;margin-right:10px;margin-left:10px;'
						value='<fmt:formatDate pattern="yyyy-MM-dd hh:mm"
									value="${forum.updateDate }" />' readonly="readonly"></td>
				</tr>
				<tr>
					<td><label>카테고리</label></td>
					<td><input class="form-control" name="pcategory" style='width:100px; margin-right:10px;margin-left:10.8px;'
						value='<c:out value="${forum.pcategory}"/>' readonly="readonly"></td>
					<td><label>조회수</label></td>
					<td><input class="form-control" name="views" style='width:150px;margin-right:10px;margin-left:10px;'
						value='<c:out value="${forum.views}"/>' readonly="readonly"></td>
					<td><label>작성자</label></td>
					<td><input class="form-control" name="user_id" style='width:150px;margin-right:10px;margin-left:10px;'
				...
				<sec:authorize access="isAuthenticated()">
					...
					<button data-oper='modify' class="btn btn-default yun_btn"
<%-- 						onclick='location.href="/forum/modify?bno=<c:out value='${forum.bno}'/>"'> --%>
 				...
 				<sec:authorize access="isAuthenticated()">
					<c:if test="${pinfo.username eq forum.user_id}">
					<button data-oper='delete' class="btn btn-danger yun_btn">
						삭제</button>
					</c:if>
				</sec:authorize> 
				
					
				<button data-oper='list' class="btn btn-info yun_btn"
...
<!-- 			</div> -->
			...
			<div class="panel-heading">
				...
				<sec:authorize access="isAuthenticated()">
				<button id='addReplyBtn' class='btn btn-primary btn-xs pull-right yun_btn'>댓글 작성</button>
				</sec:authorize>
			</div>
