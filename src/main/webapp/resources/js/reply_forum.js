...
var replyService=(function(){
	...
	function getList(param, callback, error){
		...
		$.getJSON("/replies_forum/pages/"+fno+"/"+page+".json",
				function(data){
			if(callback){
	...
	function get(rno, callback, error){
		$.get("/replies_forum/"+rno+".json", function(result){
			if(callback){
				callback(result);
			}
		}).fail(function(xhr,status,err){
	...
	function displayTime(timeValue){
		...
		if(gap<(1000*60*60*24)&&(timeValue-midnight)>0){
